...
if [ -z $IsAngularRunning ] 
	then 
		...
		kill $IsAngularRunning 
		echo "The Angular server process that was running on port 4200 has been suppressed." 
fi
...
if [ -z $isSpringBootRunning ]
	then
		...
		kill $isSpringBootRunning 
		echo "The SpringBoot server process that was running on port 8080  has been suppressed." 
fi
